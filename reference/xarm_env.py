...
class XarmEnv:
    ...
    def step_joint(self, action, gripper_action=None) -> dict[str, np.ndarray]:
        """
        Pi0关节控制: action=[joint1-6], gripper_action=optional
        Pi0输出绝对关节角度，直接设置目标位置
        注意：需要在Mode 6(在线关节轨迹规划模式)下使用
        """
        if self.arm.mode != 6:
            self._clear_error_states(6)

        # Pi0模型输出绝对关节角度，直接使用
        target_angles = np.array(action).tolist()
        
        # 执行关节控制 - 使用set_servo_angle接口（Mode 6支持速度控制）
        code = self.arm.set_servo_angle(
            angle=target_angles,
            speed=1,  # 设置合理的关节速度
            is_radian=True,
            relative=False,  # 显式指定绝对位置控制
            wait=False
        )
        
        # 错误检查和重试
        while code != 0:
            print(f"Error code {code} in set_servo_angle(), retrying...")
            self._clear_error_states(6)
            code = self.arm.set_servo_angle(
                angle=target_angles,
                speed=1,
        ...
        if gripper_action is not None:
            code = self.arm.set_gripper_position(gripper_action, wait=False)
            while code != 0:
                print(f"Error code {code} in set_gripper_position(), retrying...")
                self._clear_error_states(6)
                code = self.arm.set_gripper_position(gripper_action, wait=False)
